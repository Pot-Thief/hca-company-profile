...
| `pnpm format`          | `prettier --write .`                                                          | Formats the repository. CI runs `prettier --check .` separately, so a badly formatted commit fails there                                                                                                                                                                                      |
| `pnpm test`            | `vitest run`                                                                  | Unit tests: schemas, the content loader, `channelHref`, the icon registry, every section component, and the source-scanning conventions described in `docs/ARCHITECTURE.md`. CI runs `test:coverage` rather than this, so the thresholds apply                                                |
| `pnpm test:watch`      | `vitest`                                                                      | Unit tests in watch mode                                                                                                                                                                                                                                                                      |
| `pnpm test:coverage`   | `vitest run --coverage`                                                       | Unit tests with a v8 coverage report over `src/lib` and `src/components`, held to 100% statements, branches, functions and lines by thresholds in `vitest.config.mts`. This is what CI runs, so coverage falling below 100% fails the build rather than printing a smaller number (run in CI) |
...
| `pnpm test:lighthouse` | `pnpm build && CHROME_PATH=… lhci autorun`                                    | Lighthouse CI against a production build on port 3200, budgets in `.lighthouserc.json` (run in CI)                                                                                                                                                                                            |
